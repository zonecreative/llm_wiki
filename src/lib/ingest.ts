...
export async function autoIngest(
  ...
  return withProjectLock(normalizePath(projectPath), () =>
    autoIngestImpl(projectPath, sourcePath, llmConfig, signal, folderContext, interactive, onFileWritten),
  )
}
...
async function autoIngestImpl(
  ...
  if (!analysis) {
    await streamChat(
      llmConfig,
      [
        { role: "system", content: buildAnalysisPrompt(purpose, index, sourceContext, ingestStrategy, schema) },
